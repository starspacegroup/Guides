...
describe('GitHub Auth API', () => {
		...
		it('should redirect to setup when client ID not configured', async () => {
			...
			try {
				await GET({
					...
					url: new URL('http://localhost:4255/api/auth/github')
				} as any);
	...
	describe('GET /api/auth/github/callback', () => {
		it('should redirect to login with error when no code provided', async () => {
			...
			try {
				await GET({
					url: new URL('http://localhost:4255/api/auth/github/callback'),
		...
		it('should redirect to login when OAuth not configured', async () => {
			...
			try {
				await GET({
					url: new URL('http://localhost:4255/api/auth/github/callback?code=test-code'),
					cookies: { set: vi.fn(), delete: vi.fn() },
					platform: mockPlatform
				} as any);
				expect.fail('Should have thrown redirect');
		...
		it('should handle token exchange failure', async () => {
			...
			try {
				await GET({
					url: new URL('http://localhost:4255/api/auth/github/callback?code=invalid-code'),
					cookies: { set: vi.fn(), delete: vi.fn() },
					platform: mockPlatform
				} as any);
				expect.fail('Should have thrown redirect');
		...
		it('should complete OAuth flow and set session cookie', async () => {
			...
			expect(response.headers.get('Location')).toBe('http://localhost:4255/admin'); // Owner goes to admin
		...
		it('should redirect non-owner to home', async () => {
			...
			expect(response.status).toBe(302);
			expect(response.headers.get('Location')).toBe('http://localhost:4255/'); // Non-owner goes to home
		});
