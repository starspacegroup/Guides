...
vi.mock('$app/stores', () => {
	return {
		page: readable({
			url: new URL('http://localhost:4255/setup'),
			params: {},
			route: { id: '/setup' },
			status: 200,
			error: null,
...
describe('/setup - GitHub OAuth Setup', () => {
	...
	it('should display callback URL based on current origin', () => {
		render(SetupPage);
		// The mock page store uses 'http://localhost:4255' as origin
		...
		expect(callbackUrl.textContent).toContain('localhost:4255');
	});
	...
	it('should display homepage URL based on current origin', () => {
		...
		expect(homepageUrl?.textContent).toContain('http://localhost:4255');
	});
});
